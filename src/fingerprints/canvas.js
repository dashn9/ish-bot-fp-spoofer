...
const canvas = (canvasSalt) => {
    ...
    for (const {
        ...
        // noinspection JSUnusedLocalSymbols
        bExtensionUtils.replaceWithProxy(_Canvas.prototype, "getContext", {
            apply(target, thisArg, args) {
                ...
                bExtensionUtils.variables.renderingContextWithOperators.push({
                    canvas: thisArg,
                    context,
                    contextId,
                    operators: {},
                ...
                // console.log('!!! h00k getContext context:' + (bExtensionUtils.variables.renderingContextWithOperators.length - 1) + ' args:' + Array.from(args).join('|'));
        ...
        for (const noiseOper of kNoiseOpers) {
            bExtensionUtils.replaceWithProxy(_CanvasRenderingContext2D.prototype, noiseOper, {
                apply(target, thisArg, args) {
                    bExtensionUtils.markRenderingContextOperator(thisArg, noiseOper);
        ...
        const getNoisifyCanvas = (canvas) => {
            ...
            const context =
                bExtensionUtils.variables.renderingContextWithOperators[contextIndex];
        ...
        bExtensionUtils.replaceWithProxy(_Canvas.prototype, "toDataURL", {
            apply(target, thisArg, args) {
                let canvas = thisArg;
        ...
        bExtensionUtils.replaceWithProxy(_Canvas.prototype, "toBlob", {
            apply(target, thisArg, args) {
                let canvas = thisArg;
                const { contextIndex } = bExtensionUtils.findRenderingContextIndex(canvas);
        ...
        bExtensionUtils.replaceWithProxy(_Canvas.prototype, "convertToBlob", {
            apply(target, thisArg, args) {
                let canvas = thisArg;
                const { contextIndex } = bExtensionUtils.findRenderingContextIndex(canvas);
        ...
        bExtensionUtils.replaceWithProxy(
